...
export default function Contact() {
  ...
  usePageMeta({
    title:       'Contact Cloud Krida — Tournaments & Summer Programs Support',
    description: 'Reach the Cloud Krida team about Tournament Management, Chess, Summer Programs, or general platform support — we\'d love to hear from you.',
  })

  const CATEGORIES = [
    { value: 'tournament', label: 'Tournament' },
    { value: 'chess',      label: 'Chess Program' },
    { value: 'summer',     label: 'Summer Programs' },
    { value: 'general',    label: 'General Inquiry' },
  ]

  const [fields,      setFields]      = useState({ name: '', email: '', category: 'general', message: '' })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const categoryLabel = CATEGORIES.find(c => c.value === fields.category)?.label || 'General Inquiry'
      const payload = {
        name:     fields.name.trim(),
        email:    fields.email.trim(),
        message:  fields.message.trim(),
        category: categoryLabel,
        subject:  `New ${categoryLabel} inquiry — Cloud Krida`,
      }
      ...
      setStatus('success')
      setFields({ name: '', email: '', category: 'general', message: '' })
      setTouched({})
    } catch (error) {
      ...
      <section className="contact-hero">
        ...
        >
          ...
          <motion.p className="contact-hero-subtitle" variants={fadeUp} custom={2}>
            Questions about Tournament Management, the Chess program, Summer Programs,
            or general platform support — whoever you are on Cloud Krida, we're here to help.
          </motion.p>
        </motion.div>
      </section>
                ...
                {status === 'success' ? (
                  ...
                  >
                    ...
                    <div className="contact-field">
                      <label className="contact-label" htmlFor="ct-category">
                        What is this about? <span className="contact-required">*</span>
                      </label>
                      <select
                        id="ct-category"
                        name="category"
          ...
          >
            ...
            <div className="contact-info-card">
              ...
              <p className="contact-info-body">
                Jump straight into either program — no account required to explore.
              </p>
              <div className="contact-platform-links">
                {[
                  { icon: '♟', label: 'Chess Tournaments', path: '/tournaments/chess' },
                  { icon: '🎾', label: 'Tennis',           path: '/tournaments/tennis' },
                  { icon: '🎯', label: 'Darts',            path: '/tournaments/darts' },
                  { icon: '📚', label: 'Summer Programs',  path: '/summerclasses' },
