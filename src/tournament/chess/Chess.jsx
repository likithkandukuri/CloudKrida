...
function ChessPage() {
  ...
  // ── Breadcrumbs ─────────────────────────────────────────────────────────────
  // The trailing segment (current page) below "Chess" — null on the dashboard,
  // where "Chess" itself is the current page.
  const crumbLeaf = {
    dashboard:      null,
    create:         'Create Tournament',
    csvimport:      'Import CSV',
    tournaments:    'Events & Tournaments',
    bracket:        activeTournament?.name ?? 'Bracket',
    ...
    <div className="chess-page">
      ...
      >
        ...
        <button
          ...
          onClick={view === 'dashboard' ? () => navigate('/tournaments') : handleBack}
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            <path d="M19 12H5M12 19l-7-7 7-7"/>
          </svg>
          {view === 'dashboard' ? 'Tournament Management' : 'Back'}
        </button>
        ...
        <div className="chess-topbar-title">
          ...
          <div>
            ...
            <nav className="topbar-breadcrumb" aria-label="Breadcrumb">
              <button className="topbar-crumb-link" onClick={() => navigate('/')}>Home</button>
              <span className="topbar-crumb-sep">›</span>
              <button className="topbar-crumb-link" onClick={() => navigate('/tournaments')}>Tournament Management</button>
              <span className="topbar-crumb-sep">›</span>
              {crumbLeaf ? (
                <button className="topbar-crumb-link" onClick={() => setView('dashboard')}>Chess</button>
              ) : (
                <span className="topbar-crumb-current">Chess</span>
              )}
              {crumbLeaf && (
                <>
                  <span className="topbar-crumb-sep">›</span>
                  <span className="topbar-crumb-current">{crumbLeaf}</span>
                </>
              )}
            </nav>
          </div>
